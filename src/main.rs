...
fn save_matching_text_as_toml(matching_text: &Vec<String>, toml_filename: &str) -> Result<(), Box<dyn Error>> {
    let mut data_map: HashMap<String, Vec<String>> = HashMap::new();
    ...
    // Format the TOML content manually
    let toml_data = format_toml(&data_map);
...
fn format_toml(data_map: &HashMap<String, Vec<String>>) -> String {
    let mut toml_content = String::new();

    toml_content.push_str("# This is a TOML document\n");

    for (section, values) in data_map {
